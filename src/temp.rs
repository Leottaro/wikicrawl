...
async fn main() -> Result<(), Error> {
    ...
    thread::spawn(move || {
        for sig in signals.forever() {
            ...
            if *val {
                println_and_log("\rSIGINT received, waiting for the program to stop");
                *val = false;
            } else {
                println_and_log("\rSIGINT received again, forcing the program to stop");
                std::process::exit(0);
            }
        }
    });
    ...
    while {
        ...
        *can_continue
        ...
        let mut children: Vec<tokio::task::JoinHandle<(Page, Option<Vec<String>>)>> = Vec::new();
        ...
        println_and_log(&format!("threads finished                            "));
        ...
        if found_links.is_empty() {
            ...
            if found_pages.is_empty() {
                println_and_log(&format!("No pages found"));
            } else {
                // insert new aliases
                last_query = format!(
...
async fn extract_link_info_api(
    ...
    let formatted_url = format_url_for_reqwest(&url);
    let request = format!(
...
fn format_url_for_reqwest(url: &String) -> String { //TODO:
    url.chars()
        .map(|char| match char {
            '&' => "%26".to_string(),
            _ => char.to_string(),
